...
RCT_EXPORT_METHOD(recognize
                  : (NSString *)imagePath lang
                  : (NSString *)lang options
                  : (NSDictionary *)options resolver
                  : (RCTPromiseResolveBlock)resolve rejecter
                  : (RCTPromiseRejectBlock)reject) {
  resolve(@"testinghusfjashfkh");
}

// Method available in Javascript runtime
RCT_EXPORT_METHOD(scanForText
                  : (NSString *)url coordinates
                  : (NSDictionary *)coordinates resolver
                  : (RCTPromiseResolveBlock)resolve rejecter
                  : (RCTPromiseRejectBlock)reject) {
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
                 ^{
                   ...
                   NSString *result = [self scanImageForText:image
