...
public class TesseractOcrModule extends ReactContextBaseJavaModule {
  ...
  public TesseractOcrModule(ReactApplicationContext reactContext) {
    ...
    if (!this.DATA_PATH.contains(reactContext.getPackageName())) {
      // Android 10 doesn't seem to allow Environment.getExternalStorageDirectory() to
      // work correctly so heres a fix
      this.DATA_PATH = reactContext.getExternalFilesDir(null).getAbsolutePath().toString();
    }
    this.TESS_FILES_PATH = this.DATA_PATH + File.separator + this.TESS_FILES_DIRECTORY;

    // verbose logging bc why not
    Log.v("TesseractOcr", Environment.getExternalStorageDirectory().getAbsolutePath().toString());
    Log.v("TesseractOcr", reactContext.getExternalFilesDir(null).getAbsolutePath().toString());
  }
  ...
  @ReactMethod
  public void recognize(String imageSource, final String lang, @Nullable final ReadableMap tessOptions,
  ...
  @ReactMethod
  public void recognizeTokens(String imageSource, final String lang, @Nullable final ReadableMap tessOptions,
      final Promise promise) {
  ...
  private void onProgress(int percent) {
    ...
    this.reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("onProgressChange",
        payload);
  }
  ...
  private void prepareTrainedFilesDirectory() throws IOException {
    ...
    Log.v(getName(), TESS_FILES_PATH);
    File dir = new File(TESS_FILES_PATH);
    Log.v(getName(), dir.getAbsolutePath());
    if (!dir.exists()) {
      if (!dir.mkdirs()) {
